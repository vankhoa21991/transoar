...
from transoar.data.transforms import transform_preprocessing_luna16
...
class PreProcessor:
    ...
    def __init__(
        ...
        self._preprocessing_transform = transform_preprocessing_luna16(
        ...
        os.makedirs(self._path_to_splits, exist_ok=True)
        self._splits = {
    ...
    def run(self):
        for split_name, split_paths in self._splits.items():
            ...
            for idx, case in enumerate(split_paths):
                print(case)
                print(self._path_to_dataset)

                path_image = str(self._path_to_dataset / 'raw_splitted' / 'imagesTr'/ f'{case}_0000.nii.gz')
                path_label = str(self._path_to_dataset / 'raw_splitted' / 'labelsTr' / f'{case}.nii.gz')

                assert os.path.exists(path_image), f"Path to image {path_image} does not exist."
                assert os.path.exists(path_label), f"Path to label {path_label} does not exist."

                case_name = case

                path_to_case = self._path_to_splits / split_name / case_name
                print(path_to_case)
                os.makedirs(path_to_case, exist_ok=True)
                ...
                if os.path.exists(str(path_to_case / 'data.npy')) and os.path.exists(str(path_to_case / 'label.npy')):
                    ...
                    image = np.load(str(path_to_case / 'data.npy'))
                    label = np.load(str(path_to_case / 'label.npy'))
                else:
